...
| --- | --- | --- | --- | --- |
| Senior Dev handoff helper | `plugins/senior-task/`, `profiles/senior-dev/`, `scripts/install-senior-dev-profile.sh` | implemented / partial follow-up ergonomics | The current `senior-task` plugin exposes `create_senior_task` and `senior_active_tasks` as compatibility tool names for creating/subscribing structured Senior Dev handoffs and listing active work by repo/origin with optional comments. | Active-work lookup exists. Full follow-up routing depends on the configured runtime exposing comment/unblock/requeue or equivalent follow-up actions; keep this as a live-runtime verification point. |
| Senior Dev synchronous runner | `plugins/senior-runner/`, `profiles/senior-dev/`, `scripts/test-senior-runner.sh` | implemented with local integration tests | `senior_run_coding_task` validates input, runs headless Junie CLI synchronously, writes `prompt.md`, `spec.json`, `status.json`, `events.jsonl`, `result.md`, stdout/stderr logs, records exit code and runner state, and returns artifact paths. | Senior Dev must end with exactly one final verdict: `done`, `needs-input`, or `failed`. Live sustained usage should still be verified. |
