...
| --- | --- | --- | --- | --- |
| Senior Dev Kanban task helper | `plugins/senior-task/`, `profiles/senior-dev/`, `scripts/install-senior-dev-profile.sh` | implemented / partial follow-up ergonomics | `create_senior_task` creates/subscribes Kanban tasks; `senior_active_tasks` lists active tasks by repo/origin with optional comments | Active-task lookup exists. Full p1 follow-up routing depends on the Chat Agent having a way to comment/unblock/requeue (Kanban toolset or CLI); keep this as a live-runtime verification point. PR/CI monitoring remains separate. |
| Senior Dev synchronous runner | `plugins/senior-runner/`, `profiles/senior-dev/`, `scripts/test-senior-runner.sh` | implemented with local integration tests | `senior_run_coding_task` validates input, runs OpenCode synchronously, writes `prompt.md`, `spec.json`, `status.json`, `events.jsonl`, `result.md`, stdout/stderr logs, and returns artifact paths plus verdict | Runner intentionally does not mutate Kanban; the `senior-dev` profile maps verdict to one terminal `kanban_block`. Live sustained usage should still be verified. |
