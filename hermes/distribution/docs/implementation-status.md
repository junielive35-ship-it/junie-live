...
| --- | --- | --- | --- | --- |
| Marinator delegation plugin | `docs/delegation-protocol.md`, `plugins/marinator-delegation/` | implemented | Plugin source is distributed by `hire-junie.sh`; `install-senior-dev-profile.sh` exposes `marinator_delegate` under the `senior-dev` profile's `marinator` toolset; supports Kanban linkage for Senior Dev mode | Approved OpenCode supervision boundary inside Senior Dev; normal user code tasks enter through `create_senior_task` |
