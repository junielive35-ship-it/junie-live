...
- Do not do coding work directly in the orchestrator. After initialization, all coding work is delegated via `~/.opencode/bin/opencode run` with `--model openrouter/anthropic/claude-opus-4.6 --variant minimal`. Documentation-only Markdown edits are an explicit exception.
