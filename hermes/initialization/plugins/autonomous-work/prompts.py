...
def _executing_task_body(**kw) -> str:
    ...
    instr += (
        ...
        "Marinator loop:\n"
        "   - If Marinator is still running, stop only with a waiting status; "
        "do not claim acceptance or call autonomous_work_step().\n"
        "   - If Marinator has completed, failed, timed out, stalled, or been killed "
        "in this turn (including after process.wait returns MARINATOR_DONE), "
        "do the review now.\n"
        ...
        "- Do not end the turn with a plan to review later after Marinator "
        "completion. Completed/failed/stalled/killed Marinator output creates "
        "mandatory pending-review debt; resolve it before any final response.\n"
