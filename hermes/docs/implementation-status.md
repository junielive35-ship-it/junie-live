...
| Senior Dev Kanban delegation protocol docs | implemented | `distribution/docs/delegation-protocol.md`; `docs/architecture.md`; `docs/day-to-day-routines.md` | `create_senior_task` is the Chat Agent code-changing delegation path. In the current p1 Senior path the `senior-dev` worker uses the synchronous `senior_run_coding_task` (toolset `senior_runner`) plus one terminal `kanban_block`; `marinator_delegate` remains the general/async supervised-OpenCode runtime. |
| Marinator delegation plugin | implemented | `distribution/plugins/marinator-delegation/`; `scripts/test-marinator-delegation.sh` | Profile-installed Hermes plugin; creates durable run dirs, spawns supervised OpenCode, monitors progress, emits markers, wakes orchestrator via notify_on_complete (live) or async resume (headless). It supports follow-up sessions and optional Kanban linkage for Senior Dev mode. Per-minute progress reports are observability only, not acceptance signals. |
| Senior Dev Kanban task helper | implemented | `distribution/plugins/senior-task/`; `scripts/test-senior-task.sh`; `scripts/test-senior-dev-result.sh` | Chat Agent can create one Senior Dev Kanban task and subscribe the originating chat/thread, and look up active Senior tasks via `senior_active_tasks` for follow-up routing (attach/unblock instead of duplicate). `senior_dev_task_result` remains for the legacy async path but is not used by the p1 Senior worker. |
| Senior Dev synchronous runner | implemented (local integration tests) | `distribution/plugins/senior-runner/`; `scripts/test-senior-runner.sh` | `senior_run_coding_task` (toolset `senior_runner`, senior-dev only) runs one OpenCode task synchronously in the foreground, writes Marinator-style artifacts (`spec.json`, `status.json`, `events.jsonl`, `result.md`, logs) and a normalized VERDICT block, and returns artifact paths. It never mutates Kanban; the senior-dev worker maps the verdict to one terminal `kanban_block`. |
| Senior Dev profile lane | implemented with local integration tests; live sustained usage still needs more runs | `distribution/profiles/senior-dev/`; `scripts/install-senior-dev-profile.sh`; `scripts/test-install-senior-dev.sh`; `scripts/test-dump-rehire.sh` | `senior-dev` is installed with `marinator-delegation`, `senior-task`, and `senior-runner` plugins plus required toolsets (`senior`, `senior_runner`, `kanban`, `marinator`, `terminal`, `file`). The p1 worker protocol (`SOUL.md`/`HERMES.seed.md`) is a thin synchronous adapter: `kanban_show` → `senior_run_coding_task` → one terminal `kanban_block`. `hire-junie.sh` and `rehire-junie.sh` install/update it, so fresh hire and disaster recovery include the companion worker profile. |
...
| Verify script | implemented | `hermes/scripts/verify.sh` | Runs clean-tree preflight, bash syntax, local markdown links, git whitespace, directory structure, skill-frontmatter, plugin syntax/regression checks, Senior Dev install/result/helper tests, autonomous-work tests, and initialization-gate tests. |
| OpenCode delegation model standardization | implemented | Chat Agent seed prompts/skills use `create_senior_task`; senior-dev uses `marinator_delegate`; worker does not pin OpenCode model | OpenCode model is operator-configured. Key-file permission hardening remains an accepted MVP deviation in the active profile. |
