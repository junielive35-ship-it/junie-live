...
| Autonomous Work Window plugin | implemented | `initialization/plugins/autonomous-work/` | Exposes `autonomous_work_start` and `autonomous_work_step`; creates bounded window dirs with deterministic phase transitions; installed under `autonomous` toolset. Sustained-load confidence still developing. |
