...
| Marinator delegation plugin | implemented | `initialization/plugins/marinator-delegation/` (plugin.yaml, __init__.py, tools.py, runner.py, state.py, scripts/marinator-worker.sh); `scripts/spike-marinator-headless-stall-full-loop.sh` | Profile-installed Hermes user plugin; creates durable run dirs, spawns supervised OpenCode, monitors progress, emits markers, wakes orchestrator via notify_on_complete (live) or async `hermes chat --resume` (headless); headless resume must not block supervision/control-kill polling; Kanban and cron continuation deferred |
...
| Code mutex | implemented | `hermes/initialization/scripts/code-mutex.sh` with `status`/`acquire`/`release`/`check-stale`; `--auto-recover` flag for BROKEN state (empty dir, no holder.json) added in commit `4aefe97` | File-based lock with holder metadata; atomic `mkdir` primitive; stale-by-age recovery intentionally requires manual release (see \`docs/code-mutex.md\`) |
| Hire script | implemented | `scripts/hire-junie.sh` | Creates profile, installs SOUL.md / skills / docs / seed-HERMES.md, configures model + provider + `agent.reasoning_effort` (default: medium, override: `--reasoning <level>`) |
...
| Opencode delegation model standardization | implemented | All seed prompts/skills use `marinator_delegate` for code delegation; OpenCode model is operator-configured (not pinned by Junie Live). Forced-model snippets removed in this commit (see `marinator-delegation` plugin for the canonical worker). | — |
| Autonomous Work Window plugin | implemented | `initialization/plugins/autonomous-work/` (plugin.yaml, __init__.py, tools.py, state.py, prompts.py, scripts/aw-runner.sh) | Profile-installed Hermes user plugin; exposes `autonomous_work_start` and `autonomous_work_step` tools; creates bounded window directories with deterministic phase transitions; runner drives headless AW session turns; installed by hire-junie.sh under the `autonomous` toolset; run-based tests pending |
| Telegram communication | contract-only | Hermes gateway handles this natively | Needs gateway setup per-deployment |
| Cron watchdog | contract-only | Documented in `docs/overnight-routines.md`; backlog item queued for owner approval to install a watchdog cron that calls `hermes/initialization/scripts/code-mutex.sh check-stale --auto-recover` every 15 min | Created during initialization, not pre-installed as crontab |
| Autonomous work windows | implemented; needs more real runs | Two bounded autonomous windows completed end-to-end (pre-AW-plugin). AW plugin adds deterministic state machine, dedicated session, and runner. | First sustained-load test with AW plugin pending |
| Overnight controller | implemented | Autonomous Work Window plugin replaces cron-based controller | Plugin-based, not cron-based |
| Morning report | contract-only | Documented in overnight-routines.md | Generated during finalizing phase or by watchdog |
| Backlog management | partial / Hermes-native helper exists | Minimal Hermes-native backlog helper at `initialization/plugins/autonomous-work/backlog.py` with YAML-frontmatter Markdown CRUD, profile-local path resolution, candidate detection, and status transitions. Protocol docs at `initialization/docs/backlog-protocol.md`. AW prompts now reference concrete Hermes backlog paths and rules. | No user-facing CLI or full external issue tracker. Helper is dependency-light stdlib Python designed for AW detection and prompt integration. Legacy OpenClaw state is explicitly not read. |
...
| Hypothesis generation | contract-only | Documented | LLM-driven, no dedicated script in Hermes stack (legacy OpenClaw stack has `openclaw/scripts/hypothesis-generate.sh`) |
