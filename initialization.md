...
2. Install/update the new instance's OpenClaw overnight cron jobs and generate workspace-local audit/fallback artifacts. `hire-junie.sh` does this automatically after `openclaw agents add` by calling `scripts/install-overnight-crons.sh`; the artifacts live under `.openclaw/cron/` in the initialized workspace, not in the repo root.
3. Run OpenClaw.
4. Give Junie:
