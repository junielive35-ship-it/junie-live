...
Every delegated worker must have explicit timeouts and an expected next action. `scripts/run-backlog-worker.sh` is the opencode-compatible worker boundary: it writes prompts/logs under workspace state, accepts `AUTONOMOUS_WORKER_CMD`/`--worker-cmd-template` for noninteractive workers, passes backlog item context, and blocks explicitly if no worker command/opencode CLI is available. Without a custom worker command, the default path uses OpenCode ACP: the worker starts a local `opencode serve` instance (ACP server) and executes the prompt through `opencode run --attach http://127.0.0.1:<port>` with `--model "$AUTONOMOUS_OPENCODE_MODEL" --variant "$AUTONOMOUS_OPENCODE_VARIANT" --agent "$AUTONOMOUS_OPENCODE_AGENT"` and the prompt file content as a positional message. Direct `opencode run` without ACP is not the default path; all implementation-worker execution routes through the ACP server. Defaults are `AUTONOMOUS_OPENCODE_MODEL=openrouter/anthropic/claude-opus-4.6`, `AUTONOMOUS_OPENCODE_VARIANT=low`, and `AUTONOMOUS_OPENCODE_AGENT=build`. Cron environments may have a minimal `PATH`, so default opencode discovery first honors `AUTONOMOUS_OPENCODE_BIN`, then `command -v opencode`, then executable `$HOME/.opencode/bin/opencode` before blocking clearly. Do not use unsupported opencode flags such as `--prompt-file`. In OpenRouter-backed OpenCode setups, pass the full OpenRouter model name (for example `openrouter/anthropic/claude-opus-4.6`) only when the model exists in `provider.openrouter.models` (for example `"anthropic/claude-opus-4.6": {}` in `~/.config/opencode/opencode.jsonc`), or use an OpenCode/OpenRouter alias that the provider accepts. OpenCode credentials should be available in native auth at `$XDG_DATA_HOME/opencode/auth.json` or `$HOME/.local/share/opencode/auth.json`; if that file lacks an OpenRouter API entry and `$HOME/openrouter.key` exists, the worker provisions the native auth entry from that key and also exports `OPENROUTER_API_KEY` for cron/background environments without printing it. If OpenCode reports model/provider lookup errors, the worker must block before more work is attempted and print only masked presence/path diagnostics for config, auth, and OpenRouter key locations. On timeout or stuck detection, the watchdog must:
