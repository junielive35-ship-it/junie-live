...
git -C "$repo" status --short --branch --untracked-files=all >"$state_dir/$id-status-before.txt" || true; git -C "$repo" diff --binary >"$state_dir/$id-diff-before.patch" || true; printf '{"reason":"%s"}\n' "$reason" >"$state_dir/$id.json"
