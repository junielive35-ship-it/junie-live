...
cron_line=$(grep -n '^cron add --name Junie Live overnight watchdog (junie-live) ' "$log_file" | head -1 | cut -d: -f1)
...
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert [j["id"] for j in jobs] == ["overnight-watchdog"]; assert jobs[0]["enabled"] is True' "$cron_json" || fail "cron definitions must include only enabled watchdog job"
! grep -q '"id": "overnight-controller"' "$cron_json" || fail "cron definitions must not include controller job"
...
! grep -q '^cron rm old-controller$' "$log_file" || fail "cron installer should not need old controller cleanup fixture"
...
! grep -q '^cron add --name Junie Live overnight controller (junie-live) ' "$log_file" || fail "cron installer must not add controller definition"
...
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert [j["id"] for j in jobs] == ["overnight-watchdog"]; assert jobs[0]["enabled"] is True' "$helper_json" || fail "helper definitions must include only enabled watchdog job"
! grep -q '"id": "overnight-controller"' "$helper_json" || fail "helper definitions must not include controller job"
...
! grep -Fq 'DRY-RUN: openclaw cron add --name Junie\ Live\ overnight\ controller' "$tmp/cron-helper.out" || fail "dry-run must not show controller add"
...
if [[ "$*" == "cron list --agent verify-junie --all --json" ]]; then
  printf '{"jobs":[{"id":"old-watchdog","name":"Junie Live overnight watchdog (verify-junie)","agent":"verify-junie"}]}\n'
fi
STUB
...
grep -q '^cron rm old-watchdog$' "$install_log" || fail "install mode must remove matching existing watchdog job before re-adding"
...
[[ "$(grep -c '^cron add --name Junie Live overnight controller (verify-junie) ' "$install_log")" -eq 0 ]] || fail "install mode must not add controller"
