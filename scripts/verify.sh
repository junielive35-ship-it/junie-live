...
! grep -q '"id": "morning-report"' "$cron_json" || fail "cron definitions must not include morning report job"
...
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight watchdog (junie-live) " ); b=s.find("channels add", a); assert "--disabled" not in s[a:b]' "$log_file" || fail "watchdog cron must be enabled by default"
! grep -q '^cron add --name Junie Live overnight morning-report (junie-live) ' "$log_file" || fail "cron installer must not add morning report definition"
...
! grep -q '"id": "morning-report"' "$helper_json" || fail "helper definitions must not include morning report job"
...
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight watchdog (verify-junie) " ); assert "--disabled" not in s[a:]' "$install_log" || fail "install mode watchdog must be enabled by default"
...
# Successful fake worker updates durable state.
set +e
