...
grep -qi 'scheduled morning reports are deprecated by default' "$contract" || fail "overnight contract must document scheduled report deprecation"
...
auto_test_branch="$(git rev-parse --abbrev-ref HEAD)"
auto_tmp="$(mktemp -d)"
...
AUTONOMOUS_ALLOW_DIRTY_FOR_TESTS=true ./scripts/start-autonomous-window.sh --duration 9h --workspace "$auto_tmp/workspace" --state-dir "$auto_tmp/state" --expected-branch "$auto_test_branch" --allow-main-for-tests --dry-run >"$auto_tmp/dry.out"
...
grep -q -- "--expected-branch $auto_test_branch" "$auto_tmp/dry.out" || fail "dry-run must include expected branch"
...
start_epoch=$(date +%s)
AUTONOMOUS_ALLOW_DIRTY_FOR_TESTS=true ./scripts/start-autonomous-window.sh --duration 5s --workspace "$auto_tmp/workspace" --state-dir "$auto_tmp/bg-state" --expected-branch "$auto_test_branch" --allow-main-for-tests --max-iterations 1 --iteration-timeout 5 --worker-cmd 'printf fake-worker-ok' --skip-verify --background >"$auto_tmp/bg.out"
...
case "$*" in
  ...
  "cron list --agent junie-live --all --json") printf '{"jobs":[{"jobId":"old-watchdog","name":"Junie Live overnight watchdog (junie-live)","agentId":"junie-live","enabled":false,"message":"cd /tmp/old && run"},{"id":"old-controller","name":"Junie Live overnight controller (junie-live)","agent":"junie-live"},{"job_id":"other-agent","name":"Junie Live overnight watchdog (other)","agent_id":"other"},{"id":"unrelated","name":"Not Junie Live overnight","agent":"junie-live"}]}\n'; exit 0 ;;
  cron\ add*) exit 0 ;;
  cron\ rm*) exit 0 ;;
esac
exit 0
...
[[ ! -e "$cron_tab" ]] || fail "overnight crontab fallback artifact should not be generated by hire"
...
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="overnight-controller")["enabled"] is False' "$cron_json" || fail "controller cron must be disabled by default"
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="overnight-watchdog")["enabled"] is True' "$cron_json" || fail "watchdog cron must be enabled by default"
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="morning-report")["enabled"] is False' "$cron_json" || fail "morning report cron must be disabled by default"
...
grep -q '^cron rm old-watchdog$' "$log_file" || fail "cron installer did not remove matching stale jobId job"
grep -q '^cron rm old-controller$' "$log_file" || fail "cron installer did not remove matching id job"
! grep -q '^cron rm other-agent$' "$log_file" || fail "cron installer removed another agent's matching job"
! grep -q '^cron rm unrelated$' "$log_file" || fail "cron installer removed unrelated same-agent job"
grep -q '^cron add --name Junie Live overnight controller (junie-live) ' "$log_file" || fail "cron installer did not add controller definition"
grep -q '^cron add --name Junie Live overnight watchdog (junie-live) ' "$log_file" || fail "cron installer did not add watchdog"
grep -q '^cron add --name Junie Live overnight morning-report (junie-live) ' "$log_file" || fail "cron installer did not add morning report definition"
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight controller (junie-live) " ); b=s.index("cron add --name Junie Live overnight watchdog (junie-live) " ); assert "--disabled" in s[a:b]' "$log_file" || fail "controller cron must be installed disabled by default"
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight watchdog (junie-live) " ); b=s.index("cron add --name Junie Live overnight morning-report (junie-live) " ); assert "--disabled" not in s[a:b]' "$log_file" || fail "watchdog cron must be enabled by default"
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight morning-report (junie-live) " ); b=s.find("channels add", a); assert "--disabled" in s[a:b]' "$log_file" || fail "morning report cron must be installed disabled by default"
...
[[ ! -e "$tmp/home/.openclaw/workspace-junie-live/.openclaw/cron/overnight-routines.crontab" ]] || fail "cron helper generated deprecated crontab artifact"
...
[[ ! -e "$cron_tmp/workspace/.openclaw/cron/overnight-routines.crontab" ]] || fail "cron helper dry-run generated deprecated crontab artifact"
...
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="overnight-controller")["enabled"] is False' "$helper_json" || fail "helper controller must be disabled by default"
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="overnight-watchdog")["enabled"] is True' "$helper_json" || fail "helper watchdog must be enabled by default"
python3 -c 'import json,sys; jobs=json.load(open(sys.argv[1]))["jobs"]; assert next(j for j in jobs if j["id"]=="morning-report")["enabled"] is False' "$helper_json" || fail "helper morning report must be disabled by default"
...
grep -q 'Junie\ Live\ overnight\ controller.*--disabled' "$tmp/cron-helper.out" || fail "dry-run controller add must be disabled by default"
grep -q 'Junie\ Live\ overnight\ morning-report.*--disabled' "$tmp/cron-helper.out" || fail "dry-run morning report add must be disabled by default"
...
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight controller (verify-junie) " ); b=s.index("cron add --name Junie Live overnight watchdog (verify-junie) " ); assert "--disabled" in s[a:b]' "$install_log" || fail "install mode controller must be disabled by default"
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight watchdog (verify-junie) " ); b=s.index("cron add --name Junie Live overnight morning-report (verify-junie) " ); assert "--disabled" not in s[a:b]' "$install_log" || fail "install mode watchdog must be enabled by default"
python3 -c 'import sys; s=open(sys.argv[1]).read(); a=s.index("cron add --name Junie Live overnight morning-report (verify-junie) " ); assert "--disabled" in s[a:]' "$install_log" || fail "install mode morning report must be disabled by default"
...
BACKLOG_DIR="$loop_backlog" MUTEX_DIR="$loop_mutex" AUTONOMOUS_WORKER_STATE_DIR="$loop_state/worker" OVERNIGHT_STATE_DIR="$loop_state/controller" OVERNIGHT_WORKER_CMD="$loop_tmp/drive-worker.sh" \
  ./scripts/overnight-controller.sh --state-dir "$loop_state/controller" --expected-branch "$auto_test_branch" --allow-main-for-tests --max-iterations 2 --iteration-timeout 20 --skip-verify >"$loop_tmp/controller.out" 2>"$loop_tmp/controller.err"
status=$?
set -e
...
# Blocked items older than blocked_archive_hours should be archived
...
set +e
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog-hygiene.sh --archive-days 1 --blocked-archive-hours 1 >"$tmp/hygiene-blocked.out" 2>"$tmp/hygiene-blocked.err"
status=$?
set -e
...
# Recent blocked items should NOT be archived even with short threshold
...
set +e
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog-hygiene.sh --blocked-archive-hours 1 >"$tmp/hygiene-fresh-blocked.out" 2>"$tmp/hygiene-fresh-blocked.err"
status=$?
set -e
...
# Blocked items should NOT be archived by --archive-days alone (they use --blocked-archive-hours)
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog.sh add --type hypothesis --title "Blocked uses hours" --priority 20 >/dev/null
blocked_hours_id=$(BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog.sh list --status queued 2>/dev/null | tail -1 | cut -f1)
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog.sh update "$blocked_hours_id" --status blocked >/dev/null
sed -i 's/"created_at"[[:space:]]*:[[:space:]]*"[^"]*"/"created_at": "'"$ts_old"'"/' "$hygiene_backlog/items/$blocked_hours_id.json"

set +e
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog-hygiene.sh --archive-days 1 --blocked-archive-hours 999999 >"$tmp/hygiene-blocked-hours-only.out" 2>"$tmp/hygiene-blocked-hours-only.err"
status=$?
set -e
[[ -f "$hygiene_backlog/items/$blocked_hours_id.json" ]] || fail "blocked item should NOT be archived when blocked-archive-hours is large"
# Clean up: archive it for subsequent tests
BACKLOG_DIR="$hygiene_backlog" ./scripts/backlog-hygiene.sh --blocked-archive-hours 1 >"$tmp/hygiene-blocked-cleanup.out" 2>/dev/null || true
...
mkdir -p "$wd_idle"
./scripts/overnight-watchdog.sh --state-dir "$wd_idle/state" --mutex-dir "$wd_idle/mutex" >"$tmp/wd-idle.out" 2>"$tmp/wd-idle.err"
...
AUTONOMOUS_ALLOW_DIRTY_FOR_TESTS=true ./scripts/start-autonomous-window.sh --duration 5s --workspace "$auto_tmp/workspace" --state-dir "$fix_tmp/default-window" --expected-branch "$auto_test_branch" --allow-main-for-tests --dry-run >"$fix_tmp/default-window.out"
...
AUTONOMOUS_ALLOW_DIRTY_FOR_TESTS=true ./scripts/start-autonomous-window.sh --duration 5s --workspace "$auto_tmp/workspace" --state-dir "$fix_tmp/override-window" --expected-branch "$auto_test_branch" --allow-main-for-tests --fix-retries 3 --iteration-timeout 11 --dry-run >"$fix_tmp/override-window.out"
