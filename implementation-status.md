...
Junie Live aims to be a persistent product-owning senior SWE agent for one assigned project/area, not a generic coding bot. The MVP priority is the autonomous ownership loop, also called the **Marinator** when referring to the task-solving loop itself: strategy/context → backlog or next action → mutex → delegated opencode worker → orchestrator review/fix/verification → meaningful commit/report → reflection/cleanup.
...
| Hermes-native baseline               | `hermes/README.md`; `hermes/docs/implementation-status.md`         | baseline exists; separate development | commit `ae26866` added `hermes/`                                                                                        | Keep Hermes-specific implementation under `hermes/`; root docs only need high-level awareness unless platform direction changes. |
...
| Marinator / delegated code-changing flow | `idea.md`; `day_to_day_routines.md`; seed protocols            | partial/implemented for local scripts | opencode boundary `scripts/run-backlog-worker.sh`; verify fake-worker tests                                            | Marinator is currently a named protocol across guidance/docs/scripts, not a separate module; real long-running opencode path still needs production test. |
...
| Overnight cron install for new hires | `docs/overnight-routines.md`; `initialization.md`                  | implemented for future hires          | `scripts/install-overnight-crons.sh`; `hire-junie.sh`; verify dry-run tests                                            | Watchdog enabled by default; controller/report cron definitions are not kept by default. |
...
| Controller/watchdog summaries         | `docs/overnight-routines.md`                                       | partial                               | Controller/watchdog state and logs; verify smoke tests                                                                 | Controller and watchdog preserve operational summaries in workspace-local state/logs; no separate scheduled morning report job is maintained. |
| Backlog scripts                      | `day_to_day_routines.md`                                           | partial                               | `scripts/backlog*.sh`; `next-action.sh`; verify smoke tests                                                            | Local file-backed workflow; no external issue tracker configured. Hygiene archives blocked items after `--blocked-archive-hours` (default 4h; drive uses 2h) to prevent WARNING from session accumulation. |
...
| Hypothesis generation                | `day_to_day_routines.md`                                           | partial                               | `scripts/hypothesis-generate.sh`; backlog integration; `drive.sh` blocked-hypothesis cap                               | Blocked cap (default 3) stops runaway generation when workers repeatedly fail; prevents self-referential WARNING loop. Needs real evidence loop and hygiene over time. |
| Routine health check                 | `day_to_day_routines.md`                                           | partial                               | `scripts/routine-health.sh`; verify smoke tests                                                                        | Reports blocked items separately; warns on >3 blocked. Blocked items archived by hygiene after hours (not days) to prevent session-level WARNING accumulation.             |
